...
class FlutterPluginRecord {
  ...
  ///初始化init的回调
  final _responseInitController = StreamController<bool>.broadcast();

  Stream<bool> get responseFromInit => _responseInitController.stream;

  ///开始录制 停止录制的回调监听
  final _responseController = StreamController<RecordResponse>.broadcast();

  Stream<RecordResponse> get response => _responseController.stream;

  ///音量高低的回调
  final _responseAmplitudeController =
      StreamController<RecordResponse>.broadcast();

  Stream<RecordResponse> get responseFromAmplitude =>
      _responseAmplitudeController.stream;

  ///播放状态监听
  final _responsePlayStateController = StreamController<PlayState>.broadcast();
